...
この skill は、親エージェントが `/agent_work/workspaces/` 配下の workspace と `plan.md` を owner/controller として管理し、複雑作業を安全に進めるための運用手順です。
...
- `/agent_work/workspaces/active/`、`backlog/`、`archive/` の状態確認・移動が必要な作業
...
- workspace root は `/agent_work/workspaces/` 固定
...
- workspace root を `/agent_work/workspaces/` 以外へ戻す
